...
final class NativeIntegrationTests: XCTestCase {
    ...
    func testGlacierTextRemainsLegibleOnReadingAndOpaqueFallbackSurfaces() throws {
        ...
        for appearanceName in [NSAppearance.Name.darkAqua, .accessibilityHighContrastDarkAqua] {
            ...
            let backgrounds = [V07Palette.canvas, V07Palette.surface, V07Palette.sidebar]
            for background in backgrounds {
                ...
                for foreground in [V07Palette.ink, V07Palette.muted, V07Palette.accentInk] {
    ...
    func testStatusButtonKeepsVisibleArtworkAcrossRecordingAndProcessingTransitions() throws {
        ...
        for appearanceName in [NSAppearance.Name.aqua, .darkAqua,
            ...
            for (activity, label) in transitions {
                ...
                V07Brand.updateStatusButton(button, activity: activity, shortcut: .fn)
        ...
        let resting = V07Brand.statusImage(for: .idle)
        let recording = V07Brand.statusImage(for: .recording)
        let processing = V07Brand.statusImage(for: .transcribing)
        ...
        XCTAssertTrue(resting === V07Brand.statusImage(for: .success))
        XCTAssertTrue(recording === V07Brand.statusImage(for: .starting))
        XCTAssertTrue(processing === V07Brand.statusImage(for: .delivering))
    }
    ...
    func testDevOrphanedCapturesAreRemovedWithoutTouchingStableAppFiles() throws {
        ...
        let root = files.temporaryDirectory.appendingPathComponent("V07-cleanup-test-\(UUID())", isDirectory: true)
        try files.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? files.removeItem(at: root) }
        let captures = ["V07-Dev-recording-\(UUID())"]
        let unrelated = ["V07-recording-\(UUID())", "OtherApp-recording-\(UUID())", "V07-recording-notes", "OtherApp-recording-notes", "transcripts"]
